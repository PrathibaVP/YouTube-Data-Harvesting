...
with col1:
    ...
    # st.write(
    #     '(YouTube data got stored successfully in MongoDB database**.)')
    channel_id = st.text_input('**Enter channel_id**')
    st.write('''_click below button to store your data into MongoDB database._''')
    Get_data = st.button('**Store Data into MongoDB**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyBTRCBvGzS0V8GENpOASDbTsNVL8S09YTM'
...
with col2:
    ...
    st.write('''Here, choose the channel name we need to migrate from MongoDB to MySQL Database.''')
...
connect_for_question = mysql.connector.connect(host='localhost', user='root', password='password', db='youtube_db')
cursor = connect_for_question.cursor()

# Q1
if question_tosql == '1. What are the names of all the videos and their corresponding channels?':
    cursor.execute("SELECT distinct Channel_Name, Video_Name FROM channel,video where channel.Playlist_Id = video.Playlist_Id")
...
# Q3
    ...
    cursor.execute("SELECT distinct Channel_Name, Video_Name, View_Count FROM channel, video where channel.Playlist_Id = video.Playlist_Id ORDER BY View_Count DESC LIMIT 10;")
...
# Q5
    ...
    cursor.execute("SELECT distinct Channel_Name, Video_Name, Like_Count FROM channel, video where channel.Playlist_Id = video.Playlist_Id ORDER BY Like_Count DESC;")
...
# Q6
    ...
    cursor.execute("SELECT distinct Channel_Name, Video_Name, Like_Count, Dislike_Count FROM channel , video where channel.Playlist_Id = video.Playlist_Id ORDER BY Like_Count DESC;")
...
# Q9
    ...
    cursor.execute("SELECT distinct Channel_Name, TIME_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(TIME(Duration)))), '%H:%i:%s') AS duration  FROM channel, video where channel.Playlist_Id = video.Playlist_Id GROUP by Channel_Name ORDER BY duration DESC ;")
...
# Q10
    ...
    cursor.execute("SELECT distinct Channel_Name, Video_Name, Comment_Count FROM channel, video where channel.Playlist_Id = video.Playlist_Id ORDER BY Comment_Count DESC;")
